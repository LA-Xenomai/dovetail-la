...
static ssize_t tty_write_iter(struct kiocb *iocb, struct iov_iter *from);
static ssize_t redirected_tty_write_iter(struct kiocb *iocb, struct iov_iter *from);
...
static ssize_t hung_up_tty_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	return -EIO;
}
...
static const struct file_operations console_fops = {
	.llseek		= no_llseek,
	.read		= tty_read,
	.write_iter	= redirected_tty_write_iter,
	.poll		= tty_poll,
	.unlocked_ioctl	= tty_ioctl,
	.compat_ioctl	= tty_compat_ioctl,
	.open		= tty_open,
...
static const struct file_operations hung_up_tty_fops = {
	...
	.write_iter	= hung_up_tty_write_iter,
...
static void __tty_hangup(struct tty_struct *tty, int exit_session)
{
	...
	list_for_each_entry(priv, &tty->tty_files, list) {
		filp = priv->file;
		if (tty_write_is_redirected(filp))
			cons_filp = filp;
		if (filp->f_op->write_iter != tty_write_iter)
			continue;
		closecount++;
...
int tty_write_is_redirected(struct file *filp)
{
	return filp->f_op->write_iter == redirected_tty_write_iter;
}
EXPORT_SYMBOL(tty_write_is_redirected);
 ...
 */
static inline ssize_t do_tty_write_iter(
...
/**
 *	tty_write_iter		-	write method for tty device file
 *
...
static ssize_t tty_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	ld = tty_ldisc_ref_wait(tty);
	if (!ld)
		return hung_up_tty_write_iter(iocb, from);
	if (!ld->ops->write)
		ret = -EIO;
	else
		ret = do_tty_write_iter(ld->ops->write, tty, file, from);
	tty_ldisc_deref(ld);
	return ret;
}
...
static ssize_t redirected_tty_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (p) {
		ssize_t res;
		file_start_write(p);
		res = vfs_iter_write(p, from, &p->f_pos, 0);
		file_end_write(p);
		fput(p);
		return res;
	}
	return tty_write_iter(iocb, from);
}
...
static int tioccons(struct file *file)
{
	if (!capable(CAP_SYS_ADMIN))
		return -EPERM;
	if (tty_write_is_redirected(file)) {
		struct file *f;
		spin_lock(&redirect_lock);
		f = redirect;
		redirect = NULL;
