...
static bool pdev_may_exist(struct pci_bus *bus, unsigned int device,
			   unsigned int function, int flags)
{
	if (flags & FLAG_LS2K1000) {
					  ...
					  unsigned int devfn, int where)
{
	...
	if (priv->data->flags & FLAG_DEV_HIDDEN) {
		if (!pdev_may_exist(bus, device, function, priv->data->flags))
			return NULL;
	}
...
static const struct loongson_pci_data ls2k1000_pci_data = {
	.flags = FLAG_CFG1 | FLAG_LS2K1000 | FLAG_DEV_FIX | FLAG_DEV_HIDDEN,
	.ops = &loongson_pci_ops,
};

static const struct loongson_pci_data ls2k500_pci_data = {
...
static const struct of_device_id loongson_pci_of_match[] = {
	{ .compatible = "loongson,ls2k1000-pci",
		.data = &ls2k1000_pci_data, },
	{ .compatible = "loongson,ls2k500-pci",
		.data = &ls2k500_pci_data, },
